        ...
        while cur_len < max_length:
            ...
            if do_sample:
                ...
                next_words = torch.multinomial(F.softmax(scores, dim=-1),
                                               num_samples=top_n_per_beam)  # (batch_size * num_beams, TOPN_PER_BEAM)
                # Compute next scores
                ...
                beam_indices = beam_indices.repeat(batch_size, top_n_per_beam).to(next_words.device)
                next_words = next_words.view(batch_size,
                                             top_n_per_beam * num_beams)  # (batch_size, TOPN_PER_BEAM * num_beams)
                ...
                next_scores, next_words = torch.topk(_scores, top_n_per_beam * num_beams, dim=1, largest=True,
                                                     sorted=True)

            assert next_scores.size() == next_words.size() == (batch_size, top_n_per_beam * num_beams)
